...
public class Starter : BaseLauncher
{
	...
	private Process LaunchQemu()
	{
		...
		if (MosaSettings.EmulatorQEMUAcceleration)
		{
	...
	private Process LaunchVMware()
	{
		...
		sb.AppendLine(MosaSettings.EmulatorCores.ToString());

		sb.AppendLine("ide0:0.present = \"TRUE\"");

		sb.Append("ide0:0.fileName = ");
		sb.AppendLine(MosaSettings.ImageFile);
	...
	private void LaunchDebugger()
	{
		...
		sb.Append(MosaSettings.GDBPort);
		sb.Append(' ');
		sb.Append($"-image {MosaSettings.ImageFile}");
	...
	private void LaunchGDB()
	{
		...
		sb.Clear();
		sb.Append($"-d \"{MosaSettings.TemporaryFolder}\"");
		sb.Append($" -s \"{Path.Combine(MosaSettings.TemporaryFolder, Path.ChangeExtension(MosaSettings.ImageFile, ".bin")!)}\"");
		sb.Append($" -x \"{gdbScript}\"");
